...
export default function Dashboard() {
  ...
  // --- 3. FETCH HISTORY & UPDATE STATUS (MANUAL HEADERS) ---
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        // ✅ SUCCESS: Update Status Monitor to Live
        setSystemStatus((prev) => ({ ...prev, database: "Live" }));
      } catch (error) {
        console.error("Failed to fetch history:", error);
        // ❌ FAIL: Update Status Monitor to Offline
        setSystemStatus((prev) => ({ ...prev, database: "Offline" }));
      }

      // ✅ CHECK SHEETS: Verify if a spreadsheet is linked
  ...
  // --- 4. HANDLE UPLOAD (MANUAL HEADERS) ---
  const handleUpload = async (e) => {
   ...
   try {
      ...
      const res = await axios.post(`${API_BASE_URL}/upload/`, form, {
        headers: {
          ...getAuthHeaders().headers, // Spread existing auth headers
      ...
      alert(
        error.response?.data?.error ||
          "Extraction failed. Please log in again.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          <button
            ...
            className="flex items-center gap-3 w-full p-3 rounded-xl text-sm font-bold bg-indigo-600 text-white shadow-lg shadow-indigo-100 transition-all"
          >
            <LayoutDashboard size={20} /> Dashboard
          </button>
          <button
            onClick={() => navigate("/history")}
            className="flex items-center gap-3 w-full p-3 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-100 transition-all"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-[2.5rem] p-12 text-center transition-all ${
                  isDragging
                    ...
                    : "border-slate-200 bg-white hover:border-indigo-400 shadow-sm"
                }`}
              >
                <div className="w-20 h-20 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <UploadCloud
                    className={`w-10 h-10 transition-all ${loading ? "text-slate-300" : "text-indigo-600"}`}
                  />
                </div>
                ...
                <p className="text-slate-500 mb-8 text-sm">
                  Upload Invoice PDF for AI extraction
                </p>
                <input
                  type="file"
                  id="fileInput"
            ...
            <div className="space-y-6">
              <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-200">
                ...
                <h4 className="font-black text-lg leading-tight mb-2 text-white">
                  Google Sheets
                </h4>
                ...
                <button
                  ...
                  className="w-full bg-white text-indigo-600 py-3 rounded-xl text-xs font-black uppercase tracking-widest"
                >
                  Open Sheet
                </button>
              </div>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        ></div>
        <span
          className={`text-[10px] font-black uppercase tracking-widest ${isLive ? "text-emerald-500" : "text-rose-500"}`}
        >
          {status}
        </span>
      </div>
