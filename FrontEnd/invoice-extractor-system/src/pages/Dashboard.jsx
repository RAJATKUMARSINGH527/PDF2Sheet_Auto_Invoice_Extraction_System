...
export default function Dashboard() {
  ...
  useEffect(() => {
    const syncDashboard = () => {
      ...
      if (token && savedUser) {
        ...
        setHistory([]);
        setSystemStatus({
          database: "Offline (Login Required)",
          sheets: "Not Linked",
          api: "Online",
        });
      }
    ...
    window.addEventListener("userUpdated", syncDashboard);

    return () => {
      window.removeEventListener("storage", syncDashboard);
      window.removeEventListener("userUpdated", syncDashboard);
    };
  }, []);
  ...
  useEffect(() => {
    const fetchUserAndHistory = async () => {
      ...
      try {
        ...
        console.log("Backend User Data:", userRes.data);
        setCurrentUser(userRes.data);
        localStorage.setItem("user", JSON.stringify(userRes.data));
        localStorage.setItem(
          "spreadsheet_id",
          userRes.data.spreadsheetId || "",
        );
        ...
        setSystemStatus((prev) => ({
          ...prev,
          database: localStorage.getItem("token") ? "Live" : "Offline",
          sheets: sId ? "Live" : "Not Linked",
        }));
      }
    };
  ...
  const analytics = useMemo(() => {
    const totalCount = history.length;
    const totalSpendVal = history.reduce(
      (a, b) => a + (parseFloat(b.total) || 0),
      0,
    );
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    const recentInvoices = history.filter(
      (inv) => new Date(inv.createdAt) > thirtyDaysAgo,
    ).length;

    return {
      count: totalCount,
      totalSpend: totalSpendVal.toLocaleString("en-IN", {
      ...
      avgConfidence:
        history.length > 0
          ? (
              (history.reduce(
                (a, b) => a + (parseFloat(b.confidence) || 0),
                0,
              ) /
                history.length) *
              100
  ...
  // Professional Loader screen while syncing
  if (profileLoading) {
    return (
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900 relative">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 w-64 bg-white border-r border-slate-200 flex flex-col z-40 transition-transform duration-300 transform 
        ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="p-6 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
        ...
        <div className="p-4 border-t border-slate-100 mt-auto">
          {localStorage.getItem("token") ? (
            <button
              ...
              className="flex items-center gap-3 text-slate-500 hover:text-rose-600 font-bold text-sm transition-colors w-full p-2 rounded-lg hover:bg-rose-50"
            >
              <LogOut size={18} /> Logout
            </button>
          ) : (
            <button
              ...
              className="flex items-center gap-3 text-indigo-600 font-bold text-sm transition-colors w-full p-2 rounded-lg hover:bg-indigo-50"
            >
        ...
        <header className="h-20 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between shrink-0 gap-4 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3 sm:gap-4 ml-auto">
            <div className="text-right">
              ...
              <p className="text-[9px] sm:text-[10px] text-indigo-500 font-black uppercase tracking-widest leading-none">
                {localStorage.getItem("token") ? "Pro Member" : "Guest"}
              </p>
            </div>
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-sm uppercase shrink-0 border-2 border-slate-50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              >
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <UploadCloud
                    className={`w-8 h-8 sm:w-10 sm:h-10 transition-all ${loading ? "text-slate-300" : "text-indigo-600"}`}
                  />
                </div>
                <h2 className="text-lg sm:text-xl font-black text-slate-800 tracking-tight">
                  Drop Document
                </h2>
                <p className="text-slate-500 mb-6 sm:mb-8 text-xs sm:text-sm">
                  Upload PDF for AI extraction
                </p>
                <input
                  type="file"
                  id="fileInput"
              ...
              <div className="bg-white rounded-3xl sm:rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden transition-all">
                <div className="p-5 sm:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <History size={14} className="text-indigo-500" /> Recent
                    Activity
                  </h3>
                  <button
                    onClick={() => navigate("/history")}
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {filteredHistory.slice(0, 5).map((inv) => (
                              ...
                              >
                                ...
                                <td className="px-6 py-4 text-xs sm:text-sm text-slate-500 font-medium">
                                  {inv.vendor || "N/A"}
                                </td>
                                <td className="px-6 py-4 text-xs sm:text-sm font-black text-slate-900">
            ...
            <div className="space-y-6">
              <div className="bg-indigo-600 rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 text-white shadow-2xl shadow-indigo-200">
                ...
                <p className="text-indigo-100 text-xs font-medium mb-6">
                  Auto-sync is currently active for your linked workspace.
                </p>
                <button
                  onClick={() => {
              ...
              <div className="bg-white rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 border border-slate-200 shadow-sm">
                ...
                <div className="space-y-6">
                  <StatusLine
                    ...
                    isLive={systemStatus.database === "Live"}
                  />
                  <StatusLine
                    label="Sheets API"
                    status={systemStatus.sheets}
                    isLive={systemStatus.sheets === "Live"}
                  />
                  <StatusLine
                    label="AI Extractor"
                    status={systemStatus.api}
                    isLive={systemStatus.api === "Online"}
                  />
                </div>
              </div>
            </div>
...
// SHARED MINI-COMPONENTS (Same as your original)
function SidebarLink({ icon, label, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 w-full p-3 rounded-xl text-sm font-bold transition-all ${active ? "bg-indigo-600 text-white shadow-lg shadow-indigo-100" : "text-slate-500 hover:bg-slate-100"}`}
    >
      {icon} {label}
    </button>
  );
...
function StatCard({ icon, label, value, trend }) {
  return (
    <div className="bg-white p-5 sm:p-6 rounded-3xl sm:rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between transition-transform hover:scale-[1.02]">
      <div>
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-slate-50 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4">
          {icon}
        </div>
        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">
          {label}
        </p>
        <h3 className="text-xl sm:text-2xl font-black text-slate-800 mt-1 truncate">
          {value}
        </h3>
      </div>
      <p className="text-emerald-500 text-[9px] sm:text-[10px] font-bold mt-4 flex items-center gap-1">
...
function StatusLine({ label, status, isLive }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <div
          className={`w-1.5 h-1.5 rounded-full animate-pulse ${isLive ? "bg-emerald-500" : "bg-rose-500"}`}
        ></div>
        <span
          className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest ${isLive ? "text-emerald-500" : "text-rose-500"}`}
        >
          {status}
        </span>
      </div>
