...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // 🎯 DYNAMIC BACKEND URL:
      // Uses Render in production, localhost:5000 in development
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Use API_BASE_URL for login request
  ...
  const handleGoogleLogin = () => {
    // ✅ Redirects to correct backend based on environment
    ...
    <div className="h-screen flex items-center justify-center bg-[#F8FAFC] p-4 font-sans">
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100 flex flex-col w-full max-w-md overflow-hidden border border-slate-200">
        ...
        <div className="relative flex py-6 items-center px-10">
          ...
          <span className="shrink mx-4 text-[10px] font-black text-slate-300 uppercase tracking-widest">
            OR
          </span>
          <div className="grow border-t border-slate-100"></div>
        </div>
