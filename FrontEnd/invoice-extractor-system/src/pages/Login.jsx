...
//       localStorage.setItem("user", JSON.stringify(res.data.user)); // 2. CRITICAL FIX: Ensure spreadsheet_id is always set (even if empty)
...
//       console.log("🔑 [LOGIN] Session initialized. Spreadsheet ID:", sId); // 3. Smooth transition to dashboard

//       navigate("/dashboard"); // Optional: Instead of reload, consider using a Context provider for state
...
export default function Login() {
  ...
  // --- GOOGLE TOKEN CATCHER (Without Interceptor) ---
  useEffect(() => {
    ...
    if (token) {
      ...
      // Manually fetch profile because we don't have an interceptor to do it later
      axios
        ...
        .catch((err) => {
          ...
          setError("Failed to sync Google profile.");
          setLoading(false);
        });
    }
  }, [navigate]);
