...
// --- GOOGLE TOKEN CATCHER (Simplified for Mobile) ---
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const token = params.get("token");
    ...
    if (token) {
      ...
      // 2. URL clean karein bina reload kiye
      window.history.replaceState({}, document.title, "/login");
      
      // 3. Direct Dashboard bhejien (App.jsx baaki sync handle kar lega)
      navigate("/dashboard");
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Mobile Fix: reload() ki jagah direct navigate use karein
