  ...
  const saveMapping = async () => {
    ...
    try {
      ...
      const syncRes = await axios.post(
        `${API_BASE_URL}/sheets/sync`,
        syncPayload,
        getAuthHeaders()
      );

      if (syncRes.data.success) {
        alert(`✅ Success! Template saved and data synced to Google Sheets.`);
        navigate("/dashboard");
      }
    } catch (error) {
      console.error("❌ Sync Error:", error.response?.data || error.message);
      alert(error.response?.data?.msg || "Failed to finalize sync");
    } finally {
      setSaving(false);
    }
  };
