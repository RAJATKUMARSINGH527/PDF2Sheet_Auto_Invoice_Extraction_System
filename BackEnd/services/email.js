...
// Gmail/Nodemailer ki jagah Resend initialize kar rahe hain
// Isse connection timeout ki problem nahi aayegi
const resend = new Resend(process.env.RESEND_API_KEY);

exports.sendSuccessEmail = async (recipientEmail, invoiceData) => {
  try {
    // Resend API call - Ye Nodemailer se bohot fast hai
    const { data, error } = await resend.emails.send({
      from: 'PDF2Sheet Auto <onboarding@resend.dev>', // Agar domain verify hai toh apni email dalo
      to: recipientEmail,
      subject: `✔ Extraction Complete: ${invoiceData.vendor} (#${invoiceData.invoiceNo})`,
      html: `
        <!DOCTYPE html>
    ...
    console.log(`\x1b[32m✔ Resend Email sent successfully to: ${recipientEmail}\x1b[0m`);
  } catch (err) {
